...
def unknown_labels(split, dataset_config):
    ...
    if -1 in label_percentages: del(label_percentages[-1])
...
def label_mapping(unknown_labels, all_labels):
    new_label = -1 if -1 in all_labels else 0
    label_mapping = {}
    label_mapping_inv = {}
    ...
    if -1 in label_mapping: del(label_mapping[-1])
    if -1 in label_mapping_inv: del(label_mapping_inv[-1])
