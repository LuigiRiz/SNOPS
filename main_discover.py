...
parser.add_argument("--dataset", choices=["SemanticKITTI", "SemanticPOSS", "S3DIS"], default="S3DIS", type=str, help="dataset")
parser.add_argument("--dataset_config", default=None, type=str, help="dataset config file")
parser.add_argument("--voxel_size", default="0.025", type=float, help="voxel_size")
...
parser.add_argument("--clip_path", default=None, type=str, help="Checkpoint to clip distilled model. If None, clip info is not used.")
parser.add_argument("--clip_weight", default=1.0, type=float, help="Weight for the clip loss.")
...
def main(args):
    ...
    if args.dataset_config is None:
        if args.dataset == "SemanticKITTI":
            ...
            args.dataset_config = "config/s3dis_dataset.yaml"
        else:
    ...
    trainer = pl.Trainer(
        ...
        check_val_every_n_epoch=5 if args.dataset == "S3DIS" else 1
    )
    trainer.fit(model)
